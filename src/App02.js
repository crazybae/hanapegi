...
class App extends Component {
  ...
  runGame = async () => {
    ...
    this.runRPSMinusOne(this.account, this.makePairs(), appProp)
  }


  runRPSMinusOne = (address, pairs, appProp) => {
    // const [choice, setChoice] = useState(false);
    const authToken = "DUMMYTOKEN"
    csws = new WebSocket(appProp.gameServerURL, authToken);
    ...
    csws.onmessage = (event) => {
      ...
      switch (recData.cmd) {
        ...
        case FINAL_RESULT:
          ...
          let myRemovedCard = this.state.myRemovedCard;
          this.setState({
            myRemovedCard: 0,
          });
    ...
    // useEffect(() => {
    //   if (this.state.myChoice == 1) {
    //     console.log('MY Choice is', this.state.myChoice);
    //     this.setState({
    //       myChoice: 0,
    //       showChoiceButtons: false,
    //     });
    //   } else if (this.state.myChoice == 2) {
    //     console.log('MY Choice is', this.state.myChoice);
    //     this.setState({
    //       myChoice: 0,
    //       showChoiceButtons: false,
  ...
  chooseFirst = () => {
    ...
    csws.send(JSON.stringify({ cmd: REMOVE_CARD, removed_card_num: 1 }));
  }
  ...
  chooseSecond = () => {
    ...
    csws.send(JSON.stringify({ cmd: REMOVE_CARD, removed_card_num: 2 }));
  }
          ...
          <tr>
            ...
            <td>
              <button id='chooseFirstBtn'
                ...
                onClick={this.chooseFirst} disabled={!this.state.showChoiceButtons} >
                빼기
              </button>
            </td>
            <button id='chooseSecondBtn'
              class="bg-red-500  hover:bg-red-700 ..."
              onClick={this.chooseSecond} disabled={!this.state.showChoiceButtons} >
              빼기
            </button>
            <td>
            </td>
